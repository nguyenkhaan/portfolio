...
*Lưu ý:* Em luôn để relevant resources phía dưới mỗi chương ạ.
...
*Lưu ý*: Quá trình dịch và Quá trình chạy được đề cập ở trên là khác nhau (chi tiết xem mục 1.3)
...
1. **Kiểm tra cú pháp (Syntax Checking):** Compiler đi qua các dòng code tuần tự. Nếu phát hiện sai sót, chúng sẽ tự động báo lỗi cùng với dòng có lỗi tương ứng, lập trình viên phải sửa lỗi (`debug`) thì chương trình mới chạy tiếp được.  
    ...
    *(try it yourself - Ví dụ cố tình viết sai cú pháp để minh họa báo lỗi)*
    ```cpp 
    #include <iostream>
